...
This project supports local development hot reload (HMR) using Vite.
...
This starts a local Vite server (http://127.0.0.1:5173).

### 3. Install Development Loader

Install [`Search-Engine-Result-Hider_dev.user.js`](Search-Engine-Result-Hider_dev.user.js) in your Userscript manager (e.g. Tampermonkey).

**Note**: Make sure to **disable** or **remove** the production script `Search-Engine-Result-Hider_autoupdate.user.js` to avoid conflicts during development.

### 4. Start Developing

1. Ensure the Vite development server is running (`npm run dev`).
2. Open any supported search engine page (Google, Bing, etc.).
3. The development loader will automatically fetch the latest script from the Vite server.
4. Modify [`Search-Engine-Result-Hider_autoupdate.user.js`](Search-Engine-Result-Hider_autoupdate.user.js) and save. The page will reload automatically.

### 5. Distribution

Simply distribute [`Search-Engine-Result-Hider_autoupdate.user.js`](Search-Engine-Result-Hider_autoupdate.user.js). No build step is required. It is fully self-contained and ready for installation.
D