...
4. 订阅自动更新同样全站后台运行；多标签页同时打开时每个到期订阅仅由一个标签页拉取
...
通过脚本管理器菜单`🖋️自定义选择器`打开编辑面板（JS 格式，与内置 `const SELECTORS` 结构一致），可覆盖6个内置引擎或新增自定义引擎，保存后立即生效，无需等待脚本更新
...
3. 引擎ID仅允许字母/数字/`_`/`-`，`other` 为保留键，与内置引擎同ID（`google`/`bing`/`duckduckgo(ddg)`/`yandex`/`brave`/`yahoo(yahoo-japan)`）或站点重叠时会覆盖内置选择器，如匹配 `cn.bing.com` 时将优先于内置 `bing` 命中；多个自定义引擎之间按编辑面板中的书写顺序匹配
4. 保存时仅存储与内置有差异的键，未改动的内置不会写入存储；删除某个内置块或把覆盖改回与内置完全相同再保存即删除该覆盖，恢复跟随脚本更新；如需停用某个内置引擎，在其块内保留 `disabled: true` 再保存，重置按钮可一键恢复全部内置
...
将test文件夹和脚本放至同一目录运行，测试会自动读取上一级目录的 `.js` 脚本，按功能域分为：条件表达式、规则、选择器与引擎、跨域权限；其中选择器与引擎包含跨标签页同步锁（订阅/WebDAV 去重）测试
