...
**关于Webdav：**  
1.每小时同步一次，自动同步会根据文件时间去重合并，手动上传/下载则为覆盖文件。目前仅在单设备同步上测试过，理论上可以和ublacklist同步同一配置文件，但不保证不会出现文件错误替换逻辑问题，请提前备份文件  
2.Webdav同步依赖跨域请求权限  
...
这将启动一个本地 Vite 服务器 (http://127.0.0.1:5173)。

### 3. 安装开发加载器

在油猴中安装 [`Search-Engine-Result-Hider_dev.user.js`](Search-Engine-Result-Hider_dev.user.js)。

**注意**：请确保同时**禁用**或**删除**原有的 `Search-Engine-Result-Hider_autoupdate.user.js`，以避免脚本冲突。

### 4. 开始开发

1. 确保 Vite 开发服务器正在运行 (`npm run dev`)。
2. 打开任意支持的搜索引擎页面 (如 Google, Bing)。
3. 开发加载器会自动从 Vite 服务器加载最新的脚本代码。
4. 修改 [`Search-Engine-Result-Hider_autoupdate.user.js`](Search-Engine-Result-Hider_autoupdate.user.js) 并保存，浏览器会自动刷新页面以加载最新代码。

### 5. 分发

直接分发 [`Search-Engine-Result-Hider_autoupdate.user.js`](Search-Engine-Result-Hider_autoupdate.user.js) 即可，无需构建步骤。该文件是独立的、可直接安装的用户脚本。
