...
在搜索引擎中屏蔽搜索结果词条，支持Bing/Google/DuckDuckGo，支持包括ublacklist规则在内的JS正则匹配URL、标题匹配、正文内容匹配，支持导入导出规则到剪贴板。规则类似ublacklist，主要倾向于标题和正则匹配  
...
添加规则时可不用`*://*.`前缀直接写域名，此用于兼容ublacklist规则  
适用于仅支持安装脚本的移动端，其他支持安装扩展的移动端和PC端也可用但更推荐直接使用ublacklist
